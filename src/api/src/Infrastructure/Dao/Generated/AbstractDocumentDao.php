 ...
 * If you need to perform changes, edit the DocumentDao class instead!
 */
...
/**
 * The AbstractDocumentDao class will maintain the persistence of Document class
 * into the documents table.
 */
abstract class AbstractDocumentDao
{
    ...
    /**
     * Persist the Document instance.
     *
     * @param Document $obj The bean to save.
     */
    public function save(\App\Domain\Model\Document $obj) : void
    {
     ...
     *
     * @return \App\Domain\Model\Document[]|\TheCodingMachine\TDBM\ResultIterator
     */
    public function findAll() : \TheCodingMachine\TDBM\ResultIterator
    {
        if ($this->defaultSort) {
            $orderBy = 'documents.'.$this->defaultSort.' '.$this->defaultDirection;
        } else {
            $orderBy = null;
        }
        return $this->tdbmService->findObjects('documents', null, [], $orderBy);
    }

    /**
     * Get Document specified by its ID (its primary key).
     *
     ...
     */
    public function getById(string $id, bool $lazyLoading = false) : \App\Domain\Model\Document
    {
        return $this->tdbmService->findObjectByPk('documents', ['id' => $id], [], $lazyLoading);
    }

    /**
     * Get all Document records.
     *
     * @param \App\Domain\Model\Document $obj The object to delete
     * @param bool $cascade If true, it will delete all objects linked to $obj
     */
    public function delete(\App\Domain\Model\Document $obj, bool $cascade = false) : void
    {
        if ($cascade === true) {
    ...
    protected function find($filter = null, array $parameters = [], $orderBy = null, array $additionalTablesFetch = [], ?int $mode = null) : \TheCodingMachine\TDBM\ResultIterator
    {
        ...
        return $this->tdbmService->findObjects('documents', $filter, $parameters, $orderBy, $additionalTablesFetch, $mode);
    }

    /**
     * Get a list of Document specified by its filters.
     *
    ...
    protected function findFromSql(string $from, $filter = null, array $parameters = [], $orderBy = null, array $additionalTablesFetch = [], ?int $mode = null) : \TheCodingMachine\TDBM\ResultIterator
    {
        ...
        return $this->tdbmService->findObjectsFromSql('documents', $from, $filter, $parameters, $orderBy, $mode);
    }

    /**
     * Get a list of Document from a SQL query.
     *
    ...
    {
        return $this->tdbmService->findObjectsFromRawSql('documents', $sql, $parameters, $mode, null, $countSql);
    }

    /**
     * Get a single Document specified by its filters.
     *
     * @param mixed $filter The filter bag (see TDBMService::findObjects for complete description)
     * @param mixed[] $parameters The parameters associated with the filter
     * @param string[] $additionalTablesFetch A list of additional tables to fetch (for performance improvement)
     * @return \App\Domain\Model\Document|null
     */
    protected function findOne($filter = null, array $parameters = [], array $additionalTablesFetch = []) : ?\App\Domain\Model\Document
    {
        return $this->tdbmService->findObject('documents', $filter, $parameters, $additionalTablesFetch);
    }
     ...
     * @return \App\Domain\Model\Document|null
     */
    protected function findOneFromSql(string $from, $filter = null, array $parameters = []) : ?\App\Domain\Model\Document
    {
        return $this->tdbmService->findObjectFromSql('documents', $from, $filter, $parameters);
    }
