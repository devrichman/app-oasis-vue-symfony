 ...
 * If you need to perform changes, edit the EventDao class instead!
 */
...
/**
 * The AbstractEventDao class will maintain the persistence of Event class into the
 * events table.
 */
abstract class AbstractEventDao
{
    ...
    /**
     * Persist the Event instance.
     *
     * @param Event $obj The bean to save.
     */
    public function save(\App\Domain\Model\Event $obj) : void
    {
     ...
     *
     * @return \App\Domain\Model\Event[]|\TheCodingMachine\TDBM\ResultIterator
     */
    public function findAll() : \TheCodingMachine\TDBM\ResultIterator
    {
        if ($this->defaultSort) {
            $orderBy = 'events.'.$this->defaultSort.' '.$this->defaultDirection;
        } else {
            $orderBy = null;
        }
        return $this->tdbmService->findObjects('events', null, [], $orderBy);
    }

    /**
     * Get Event specified by its ID (its primary key).
     *
     ...
     */
    public function getById(string $id, bool $lazyLoading = false) : \App\Domain\Model\Event
    {
        return $this->tdbmService->findObjectByPk('events', ['id' => $id], [], $lazyLoading);
    }

    /**
     * Get all Event records.
     *
     * @param \App\Domain\Model\Event $obj The object to delete
     * @param bool $cascade If true, it will delete all objects linked to $obj
     */
    public function delete(\App\Domain\Model\Event $obj, bool $cascade = false) : void
    {
        if ($cascade === true) {
    ...
    protected function find($filter = null, array $parameters = [], $orderBy = null, array $additionalTablesFetch = [], ?int $mode = null) : \TheCodingMachine\TDBM\ResultIterator
    {
        ...
        return $this->tdbmService->findObjects('events', $filter, $parameters, $orderBy, $additionalTablesFetch, $mode);
    }

    /**
     * Get a list of Event specified by its filters.
     *
    ...
    protected function findFromSql(string $from, $filter = null, array $parameters = [], $orderBy = null, array $additionalTablesFetch = [], ?int $mode = null) : \TheCodingMachine\TDBM\ResultIterator
    {
        ...
        return $this->tdbmService->findObjectsFromSql('events', $from, $filter, $parameters, $orderBy, $mode);
    }

    /**
     * Get a list of Event from a SQL query.
     *
    ...
    {
        return $this->tdbmService->findObjectsFromRawSql('events', $sql, $parameters, $mode, null, $countSql);
    }

    /**
     * Get a single Event specified by its filters.
     *
     * @param mixed $filter The filter bag (see TDBMService::findObjects for complete description)
     * @param mixed[] $parameters The parameters associated with the filter
     * @param string[] $additionalTablesFetch A list of additional tables to fetch (for performance improvement)
     * @return \App\Domain\Model\Event|null
     */
    protected function findOne($filter = null, array $parameters = [], array $additionalTablesFetch = []) : ?\App\Domain\Model\Event
    {
        return $this->tdbmService->findObject('events', $filter, $parameters, $additionalTablesFetch);
    }
     ...
     * @return \App\Domain\Model\Event|null
     */
    protected function findOneFromSql(string $from, $filter = null, array $parameters = []) : ?\App\Domain\Model\Event
    {
        return $this->tdbmService->findObjectFromSql('events', $from, $filter, $parameters);
    }
